...
const ListingPage = () => {
  ...
  const handleSearch = () => {
    const inputAddress = document.getElementById('searchInput').value
    // console.log(typeof)
    const regex = new RegExp(inputAddress, 'i');
    const filteredAddresses = allHouses.filter(item => regex.test(item.address));
    setFilterHouses(filteredAddresses);
    // const sendingData = JSON.stringify({address : inputAddress})
    
    // fetch('http://localhost:8000/api/getHome', {
    //   method:"POST",
    //   mode: "cors",
    //   headers: {
                ...
                <div className="flex md:flex-col flex-row gap-5 items-start justify-start w-full">
                  <div className="bg-white-A700 border border-bluegray-100 border-solid flex flex-1 flex-col h-[60px] md:h-auto items-start justify-start px-4 py-3.5 rounded-[10px] w-full">
                    <Input
                      ...
                      wrapClassName="flex pt-1 w-auto sm:w-full" id="searchInput"
                      suffix={
                        <Img
              ...
              <div className="flex flex-1 flex-col md:gap-10 gap-[60px] items-start justify-start w-full">
                <div className="flex flex-col items-start justify-start w-full">
                  <div className="md:gap-5 gap-6 grid md:grid-cols-1 grid-cols-2 justify-center min-h-[auto] w-full">
                    ...
                    {
                      filterHouses.map(house => <LandingPageCard key={house._id} details ={house}/>)
                    }
                  </div>
                </div>
